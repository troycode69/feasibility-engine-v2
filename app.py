...
st.set_page_config(page_title="STORAGE OS", page_icon="🏢", layout="wide")

# === STORAGE OS BRAND CSS (STRICT READABILITY) ===
st.markdown("""
<style>
    ...
    h1, h2, h3, h4, h5, h6 { color: #0C2340 !important; font-weight: 700 !important; }
    [data-testid="stSidebar"] { background-color: #0C2340; }
    [data-testid="stSidebar"] * { color: white !important; }
    .stButton>button[kind="primary"] { background-color: #F39C12 !important; color: white !important; font-weight: 600 !important; }
...
# === PAGE 2: MARKET INTEL ===
    ...
    # === PDF UPLOAD (Multi) ===
    st.markdown("---")
    st.markdown("### 📄 Market Report Upload (Up to 4 PDFs)")
    
    pdf_files = st.file_uploader("Upload Market Research PDFs", type=['pdf'], accept_multiple_files=True)
    if pdf_files:
        if len(pdf_files) > 4:
            st.warning("Maximum 4 PDFs allowed. Using first 4.")
        ...
        for pdf in pdf_files:
            ...
            with st.spinner(f"Analyzing {pdf.name}..."):
                ext_result = extract_pdf_data(pdf)
                st.session_state.pdf_ext_data[pdf.name] = ext_result
                if ext_result['extracted_rates']:
                    st.toast(f"📈 Found {len(ext_result['extracted_rates'])} rates in PDF")
        ...
        if 'ai_results' in st.session_state and st.session_state.ai_results:
            ...
            with col_demo:
                ...
                if 'demographics' in st.session_state.ai_results:
                    ...
                    st.write(f"**Age 25-54:** {demo_data['age_25_54_pct']}%")
    ...
    with st.expander("👥 Demographics (25 points)", expanded=True):
        # Auto-fill age from AI if available
        default_age_pct = 40
        ...
        if 'ai_results' in st.session_state and 'demographics' in st.session_state.ai_results:
            ...
            st.success(f"✅ AI values loaded from Census API - Age 25-54% updated to real data")

        col1, col2 = st.columns(2)
        with col1:
            pop_3mi = st.number_input("Population (3mi)", value=50000, step=5000)
            income = st.number_input("Median Income ($)", value=65000, step=5000)
            growth = st.number_input("Growth Rate (%)", value=2.0, step=0.5)
        with col2:
            renter_pct = st.number_input("Renter %", value=35, step=5)
        ...
        with st.spinner("Generating Bob Copper-quality PDF report..."):
            try:
                ...
                report_data = {
                    ...
                    'demographics': {
                        'population': 50000,  # From scoring inputs
                        'median_income': 65000,
                        'household_growth': 0.02,
                        'renter_occupied_pct': 35
                    },
                    'feasibility': {
                        'sqft_per_capita': 5.5  # From scoring inputs
                    },
                    'competitors': st.session_state.get('all_competitors', [])[:10],
                    'comp_stats': {
                        'count': len(st.session_state.get('all_competitors', [])),
                        'avg_occupancy': 87,
                        'avg_rate_10x10': 120
                    },
